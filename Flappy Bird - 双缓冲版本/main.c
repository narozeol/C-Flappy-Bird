...
//重置缓冲区函数
void ResetBuffer(int buffer[][SCREENHEIGHT])
{
...
//设置缓冲区
void SetBuffer (int buffer[][SCREENHEIGHT])
{
    //重置缓冲区
    ResetBuffer (buffer);
    //画鸟
...
//释放缓冲区
void GameDraw (int buffer[][SCREENHEIGHT])
{
    for (int i = 0; i < SCREENWIDTH; i++)
        for (int j =0; j < SCREENHEIGHT; j++)
        {
...
void BarrierMove (int speed)
{
    for (int i = 0; i < BARRIER_MAX; i++)
    {
        ...
        if (barrier[i].x <= 0-BARRIER_WIDTH)
        {
            barrier[i].live = 0;
        }
    }
...
start:
    ...
    while (1)
    {
        ...
        {
            SetBuffer (buffer1);
            GameDraw (buffer2);//画出图像
            flag = -flag;
        }
        else
        {
            SetBuffer (buffer2);
            GameDraw (buffer1);//画出图像
            flag = -flag;
        }
